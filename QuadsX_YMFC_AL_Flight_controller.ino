...
#define V1_2_FIX 1                 // integration of Joop's 1.2 fix 

#define USE_FASTER_ISR_HANDLER 1   // comment this to return to Joop's original ISR handler

#define RETURN_FROM_ISR 1          // performance update for the ISR Handler - comment to remove
...
static float pid_p_gain_roll = 1.3;               //Gain setting for the roll P-controller
static float pid_i_gain_roll = 0.03; // 0.022;  // 0.02;     //0.04         //Gain setting for the roll I-controller - angular motion
static float pid_d_gain_roll = 16.7; //17.0;  // 18.0;             //Gain setting for the roll D-controller - reacts to change of error
static int pid_max_roll = 400;                    //Maximum output of the PID-controller (+/-)

static float pid_p_gain_pitch = pid_p_gain_roll;  //Gain setting for the pitch P-controller.
static float pid_i_gain_pitch = pid_i_gain_roll;  //Gain setting for the pitch I-controller.
static float pid_d_gain_pitch = pid_d_gain_roll;  //Gain setting for the pitch D-controller.
static int pid_max_pitch = pid_max_roll;          //Maximum output of the PID-controller (+/-)

static float pid_p_gain_yaw = 4.0;                //Gain setting for the pitch P-controller. //4.0
static float pid_i_gain_yaw = 0.02;               //Gain setting for the pitch I-controller. //0.02
static float pid_d_gain_yaw = 0.0;                //Gain setting for the pitch D-controller.
static int pid_max_yaw = 400;                     //Maximum output of the PID-controller (+/-)


/*
float pid_p_gain_roll = 1.3;               //Gain setting for the roll P-controller
float pid_i_gain_roll = 0.03; // 0.022;  // 0.02;     //0.04         //Gain setting for the roll I-controller - angular motion
float pid_d_gain_roll = 16.8; //17.0;  // 18.0;             //Gain setting for the roll D-controller - reacts to change of error
...
static int receiver_input_channel_1, receiver_input_channel_2, receiver_input_channel_3, receiver_input_channel_4;
static int counter_channel_1, counter_channel_2, counter_channel_3, counter_channel_4, loop_counter;

static int esc_1, esc_2, esc_3, esc_4;
static int throttle, battery_voltage;
static int cal_int, start, gyro_address;
static int receiver_input[5];

int temperature;
static int acc_axis[4], gyro_axis[4];
...
// added to resolve issues with endpoints on dx6i (and others)

#define RECEIVER_PPM_THROTTLE_MIN_AVAILABLE 1150    // should be 1000
#define RECEIVER_PPM_YAW_MIN_AVAILABLE 1150         // should be 1000
#define RECEIVER_PPM_YAW_CTR_AVAILABLE 1400         // should be <1500
#define RECEIVER_PPM_YAW_MAX_AVAILABLE 1900         // should be 2000

#define BATTERY_STEP   3450      // default was 3500
...
void loop(){
...
#ifdef V1_2_FIX
  if(labs(acc_y) < acc_total_vector)
#endif
     angle_pitch_acc = asin((float)acc_y/acc_total_vector)* 57.296;            //Calculate the pitch angle.
#ifdef V1_2_FIX
  if(labs(acc_x) < acc_total_vector)
#endif
  ...
  if(receiver_input_channel_3 < (int)RECEIVER_PPM_THROTTLE_MIN_AVAILABLE && receiver_input_channel_4 < (int)RECEIVER_PPM_YAW_MIN_AVAILABLE)
     start = 1;
  //When yaw stick is back in the center position start the motors (step 2).
  //if(start == 1 && receiver_input_channel_3 < 1050 && receiver_input_channel_4 > 1450){
  if(start == 1 && receiver_input_channel_3 < (int)RECEIVER_PPM_THROTTLE_MIN_AVAILABLE && receiver_input_channel_4 > (int)RECEIVER_PPM_YAW_CTR_AVAILABLE){
     start = 2;
  ...
  if(start == 2 && receiver_input_channel_3 < (int)RECEIVER_PPM_THROTTLE_MIN_AVAILABLE && receiver_input_channel_4 > (int)RECEIVER_PPM_YAW_MAX_AVAILABLE)
     start = 0;
  ...
  if (start == 2){                                                          //The motors are started.
    ...
    if (battery_voltage < 1240 && battery_voltage > 800){                   //Is the battery connected?
      esc_1 += esc_1 * ((1240 - battery_voltage)/(float)BATTERY_STEP);              //Compensate the esc-1 pulse for voltage drop.
      esc_2 += esc_2 * ((1240 - battery_voltage)/(float)BATTERY_STEP);              //Compensate the esc-2 pulse for voltage drop.
      esc_3 += esc_3 * ((1240 - battery_voltage)/(float)BATTERY_STEP);              //Compensate the esc-3 pulse for voltage drop.
      esc_4 += esc_4 * ((1240 - battery_voltage)/(float)BATTERY_STEP);              //Compensate the esc-4 pulse for voltage drop.
    } 
